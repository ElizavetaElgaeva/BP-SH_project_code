# Aim of this script is to filter the results of SMR-HEIDI analysis for SH
...
tissues <- fread("matching_tissues.txt", data.table = F)
...
fwrite(merged_full, file = "sh_results_full_preselected_tissues.txt", dec = ".", sep = "\t")
...
fwrite(merged_filtered, file = "sh_results_filtered_columns.txt", dec = ".", sep = "\t")

# Let's filter the SNPs. Select only replicated
merged_fin <- merged_filtered[merged_filtered$locus == "rs2587363" | merged_filtered$locus == "rs11079993", ]
fwrite(merged_fin, file = "sh_results_repl_only.txt", dec = ".", sep = "\t")
# set a p_smr threshold: 0.05/N, where N is the number of unique combinations of tissue-probe for the two replicated SNPs
thr_smr <- 0.05/length(unique(merged_fin$gwas_2))
thr_heidi <- 0.01 # standard threshold
table(merged_fin$p_smr < thr_smr)
table(merged_fin$p_heidi >= thr_heidi)
table(merged_fin$p_heidi >= thr_heidi & merged_fin$p_smr < thr_smr)
ind <- which(merged_fin$p_heidi >= thr_heidi & merged_fin$p_smr < thr_smr)
fwrite(merged_fin[ind, ], file = "sh_results_passed_thr.txt", dec = ".", sep = "\t")
