...
thr <- 0.05/(3*(length(bp_test_nonr_var1_cov) + length(opcs_bp_test_nonr_var1) + 6)) # 5.50055e-05; 6 - is a number of pain traits, glm results for them were added later
...
readme_bp_icd <- "glm() results for ICD10 codes level 2, chapter I - XVII only, against BP PRS, significant"
save(readme_bp_icd, or_bp_icd, bp_icd, file = "./glm_icd_level_2_chapter_1-17_bp_prs_filtered_test_nonrelatives.RData")
...
readme_bp_opcs <- "glm() results for OPCS codes level 2 (X, Y, Z OPCS codes excluded) against BP PRS, significant"
save(readme_bp_opcs, or_bp_opcs, bp_opcs, file = "./glm_opcs_level_2_no_xyz_bp_prs_filtered_test_nonrelatives.RData")
