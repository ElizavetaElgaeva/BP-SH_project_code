...
# (Whole sample, cases, non-relatieves only; standardized PRS; ICD10 and OPCS codes not-combined)
...
# SH PRS
load("icd_ch1-17_vs_10_sh_all_nonr_cases_signif.RData")
load("icd_ch1-17_vs_90_sh_all_nonr_cases_signif.RData")

load("opcs_no_xyz_vs_10_sh_all_nonr_cases_signif.RData")
load("opcs_no_xyz_vs_90_sh_all_nonr_cases_signif.RData")

# BP-SH PRS
load("icd_ch1-17_vs_10_bp_sh_all_nonr_cases_signif.RData")
load("icd_ch1-17_vs_90_bp_sh_all_nonr_cases_signif.RData")

load("opcs_no_xyz_vs_10_bp_sh_all_nonr_cases_signif.RData")
load("opcs_no_xyz_vs_90_bp_sh_all_nonr_cases_signif.RData")

ls()

# Select ICD10 and OPCS codes of interest
codes <- rownames(icd_vs_10_sh_sign)
codes <- c(codes, rownames(icd_vs_90_sh_sign))
codes <- c(codes, rownames(icd_vs_10_bp_sh_sign))
codes <- c(codes, rownames(icd_vs_90_bp_sh_sign))
codes <- unique(codes)

codes2 <- rownames(opcs_vs_10_sh_sign)
codes2 <- c(codes2, rownames(opcs_vs_90_sh_sign))
codes2 <- c(codes2, rownames(opcs_vs_10_bp_sh_sign))
codes2 <- c(codes2, rownames(opcs_vs_90_bp_sh_sign))
codes2 <- unique(codes2)

intersect(codes, codes2) # 0
all_codes <- c(codes, codes2) # length(codes) = 84, length(codes2) = 34
...
sh_10_full <- c(if_10_sh_vs_icd[codes], if_10_sh_vs_opcs[codes2])
...
sh_90_full <- c(if_90_sh_vs_icd[codes], if_90_sh_vs_opcs[codes2])
...
bp_sh_10_full <- c(if_10_bp_sh_vs_icd[codes], if_10_bp_sh_vs_opcs[codes2])
...
bp_sh_90_full <- c(if_90_bp_sh_vs_icd[codes], if_90_bp_sh_vs_opcs[codes2])
...
load("//home/common/projects/pain_project/UKBB_pheno_ICD10_OPCS/prev_of_icd10_cases_all_nonrel_filtered.RData")
load("/home/common/projects/pain_project/UKBB_pheno_ICD10_OPCS/prev_opcs_cases_all_nonrel_filtered.RData")
prev_full <- c(prev_icd_cases_f[codes], prev_opcs_cases_f[codes2])
tab$"code_prevalence" <- prev_full

fwrite(tab, file = "glm_results_preselected_codes_nonrel_all_cases.txt", sep = "\t", dec = ".")
