# Aim of this script is to obtain GWAS summary statistics for shared heredity in EA Replication cohort

library(data.table)
library(dplyr)
source("../../shared_heredity/00_core_functions/linear_combination_v3.R") 

path_to_result_directory <- '../../../data/01_sh/sh_replication/EA/00_raw_data/'

path_to_pain_gwas <- '/mnt/polyomica/projects/mv_gwas/data/chronic_replication/EA_09102018/unification_results/'

gwas_files <- c(paste0(path_to_pain_gwas, 'Hip_output_done.csv'),
		paste0(path_to_pain_gwas, 'Back_output_done.csv'),
		paste0(path_to_pain_gwas, 'Neck_output_done.csv'),
		paste0(path_to_pain_gwas, 'Knee_output_done.csv'),
		paste0(path_to_pain_gwas, 'Head_output_done.csv'),
		paste0(path_to_pain_gwas, 'Stom_output_done.csv'))
...
data.table::fwrite(
	sh_gwas, 
	file = paste0(path_to_result_directory, 'SH_EA_repl_GWAS.txt'),
	sep = '\t')
