# Aim of this script is to obtain GWAS summary statistics for shared heredity in SA Replication cohort

library(data.table)
library(dplyr)
source("../../shared_heredity/00_core_functions/linear_combination_v3.R") 

path_to_result_directory <- '../../../data/01_sh/sh_replication/SA/00_raw_data/'

path_to_pain_gwas <- '/mnt/polyomica/projects/bp-sh/data/00_upload_to_db/01_replication/SA/00_original_traits/unification_results/'
...
data.table::fwrite(
	sh_gwas, 
	file = paste0(path_to_result_directory, 'SH_SA_repl_GWAS.txt'),
	sep = '\t')
